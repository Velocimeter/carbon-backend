...
export class ActivityV2Service {
  ...
  async update(endBlock: number, deployment: Deployment, tokens: TokensByAddress): Promise<void> {
    console.log(`[actdunks] Starting activity update from block ${deployment.startBlock} to ${endBlock} for ${deployment.blockchainType}`);
    ...
    console.log(`[actdunks] Last processed block: ${lastProcessedBlock}`);
    ...
    const deleteResult = await this.activityRepository
      ...
      .execute();
    console.log(`[actdunks] Cleaned up ${deleteResult.affected || 0} existing activities`);

    await this.initializeStrategyStates(lastProcessedBlock, deployment, strategyStates);
    console.log(`[actdunks] Initialized ${strategyStates.size} strategy states`);
    ...
    for (let batchStart = lastProcessedBlock; batchStart < endBlock; batchStart += this.BATCH_SIZE) {
      ...
      console.log(`[actdunks] Processing batch from block ${batchStart} to ${batchEnd}`);
      ...
      console.log(`[actdunks] Found events in batch: created=${createdEvents.length}, updated=${updatedEvents.length}, deleted=${deletedEvents.length}, transfer=${transferEvents.length}`);
      ...
      console.log(`[actdunks] Generated ${activities.length} activities from events`);
      ...
      for (let i = 0; i < activities.length; i += this.SAVE_BATCH_SIZE) {
        ...
        console.log(`[actdunks] Saved batch of ${activityBatch.length} activities (${savedCount}/${activities.length})`);
      }

      // Update the last processed block for this batch
      await this.lastProcessedBlockService.update(key, batchEnd);
      console.log(`[actdunks] Updated last processed block to ${batchEnd}`);
    }
    console.log(`[actdunks] Completed activity update for ${deployment.blockchainType}`);
  }

  async getFilteredActivities(params: ActivityDto | ActivityMetaDto, deployment: Deployment): Promise<ActivityV2[]> {
    console.log(`[actdunks] Getting filtered activities for ${deployment.blockchainType} with params:`, params);
    ...
    console.log(`[actdunks] Found ${activities.length} activities matching filters`);
  ...
  processEvents(
    ...
    console.log(`[actdunks] Processing events for ${deployment.blockchainType}`);
    ...
    console.log(`[actdunks] Sorted ${allEvents.length} events chronologically`);

    for (const { type, event } of allEvents) {
      switch (type) {
        case 'created': {
          const createdEvent = event as StrategyCreatedEvent;
          console.log(`[actdunks] Processing create event for strategy ${createdEvent.strategyId}`);
        ...
        case 'updated': {
          ...
          if (state) {
            ...
            console.log(`[actdunks] Processing update event for strategy ${updatedEvent.strategyId}`);
        ...
        case 'deleted': {
          ...
          console.log(`[actdunks] Processing delete event for strategy ${deletedEvent.strategyId}`);
        ...
        case 'transfer': {
          ...
          console.log(`[actdunks] Processing transfer event for strategy ${transferEvent.strategyId}`);
  ...
  private determineUpdateType(event: StrategyUpdatedEvent, previousState: StrategyState): string {
    console.log(`[actdunks] Determining update type for strategy ${event.strategyId}`);
  ...
  private async initializeStrategyStates(
    ...
    console.log(`[actdunks] Initializing strategy states for ${deployment.blockchainType} up to block ${lastProcessedBlock}`);
