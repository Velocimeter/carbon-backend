...
export class StrategyService {
  ...
  async update(
    ...
    for (let block = startBlock + 1; block <= endBlock; block += deployment.harvestEventsBatchSize * 10) {
      ...
      this.logger.log(
        `CODEX_BARS StrategyService.update range=${block}-${rangeEnd} created=${createdEvents.length} updated=${updatedEvents.length} deleted=${deletedEvents.length}`,
      );
  ...
  async createOrUpdateFromEvents(
    ...
    for (let i = 0; i < strategies.length; i += BATCH_SIZE) {
      ...
      try {
        const res = await this.strategyRepository.save(batch);
        this.logger.log(
          `CODEX_BARS StrategyService.save saved=${res.length} deletionEvent=${deletionEvent} range_batch=${i}-${i + batch.length - 1}`,
        );
      } catch (e) {
        this.logger.error(`CODEX_BARS StrategyService.save error size=${batch.length} msg=${(e as any)?.message}`);
        throw e;
      }
    }
  }
